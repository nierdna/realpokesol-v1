...
import {
  IMatchQueue,
  MatchQueueEntry,
} from '../../ports/match-queue.interface';
...
export class MemoryMatchQueue implements IMatchQueue {
  ...
  async join(userId: string): Promise<void> {
    ...
    this.logger.log(
      `User joined queue: ${userId}, position: ${this.queue.length}`,
    );
  }
  ...
  async getNext(count: number): Promise<MatchQueueEntry[]> {
    ...
    if (earliestTime) {
      const sameTimeEntries = this.queue.filter(
        (e) => e.joinedAt === earliestTime,
      );
      if (sameTimeEntries.length > count) {
        // Shuffle and take only what we need
        for (let i = sameTimeEntries.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [sameTimeEntries[i], sameTimeEntries[j]] = [
            sameTimeEntries[j],
            sameTimeEntries[i],
          ];
        }
  ...
  async remove(userIds: string[]): Promise<void> {
    ...
    if (removed > 0) {
      this.logger.log(
        `Removed ${removed} users from queue: ${userIds.join(', ')}`,
      );
    }
  }
