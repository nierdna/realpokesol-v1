...
export class BattleScene extends Phaser.Scene {
  ...
  private createBattleUI() {
    ...
    const p1Name = this.add
      .text(0, 60, user?.nickname || "Player 1", {
        fontSize: "16px",
        color: "#ffffff",
        fontStyle: "bold",
      })
    ...
    const p2Name = this.add
      .text(0, 60, this.currentBattle?.opponent.nickname || "Opponent", {
        fontSize: "16px",
        color: "#ffffff",
        fontStyle: "bold",
      })
    ...
    this.turnIndicator = this.add
      .text(width / 2, height * 0.15, "Waiting for battle to start...", {
        fontSize: "20px",
        color: "#10b981",
        fontStyle: "bold",
      })
    ...
    this.actionButton = this.add
      .text(width / 2, height * 0.8, "Attack!", {
        fontSize: "24px",
        color: "#ffffff",
  ...
  private createHPBars() {
    ...
    // Player 1 HP bar
    this.add
      .text(width * 0.25, height * 0.25, "HP:", {
        fontSize: "14px",
        color: "#ffffff",
      })
      .setOrigin(0.5);
    ...
    // Player 2 HP bar
    this.add
      .text(width * 0.75, height * 0.25, "HP:", {
        fontSize: "14px",
        color: "#ffffff",
      })
      .setOrigin(0.5);
  ...
  private updateHPBars() {
    ...
    this.player1HP.fillRect(
      width * 0.25 - barWidth / 2,
      height * 0.28,
      barWidth,
      barHeight,
    );
    this.player1HP.fillStyle(
      p1Percentage > 0.5 ? 0x10b981 : p1Percentage > 0.25 ? 0xf59e0b : 0xef4444,
    );
    this.player1HP.fillRect(
      width * 0.25 - barWidth / 2,
      height * 0.28,
      barWidth * p1Percentage,
      barHeight,
    );
    ...
    this.player2HP.fillRect(
      width * 0.75 - barWidth / 2,
      height * 0.28,
      barWidth,
      barHeight,
    );
    this.player2HP.fillStyle(
      p2Percentage > 0.5 ? 0x10b981 : p2Percentage > 0.25 ? 0xf59e0b : 0xef4444,
    );
    this.player2HP.fillRect(
      width * 0.75 - barWidth / 2,
      height * 0.28,
      barWidth * p2Percentage,
      barHeight,
    );
    ...
    this.add
      .text(
        width * 0.25,
        height * 0.31,
        `${this.battleState.p1.hp}/${this.battleState.p1.maxHp}`,
        {
          fontSize: "12px",
          color: "#ffffff",
        },
    ...
    this.add
      .text(
        width * 0.75,
        height * 0.31,
        `${this.battleState.p2.hp}/${this.battleState.p2.maxHp}`,
        {
          fontSize: "12px",
          color: "#ffffff",
        },
    ...
    socketManager.on("battle.start", (data: unknown) => {
      ...
      if (!this.isDestroyed) {
        this.handleBattleStart(
          data as { roomId: string; battleState: BattleState },
        );
      }
    });
    ...
    socketManager.on("battle.end", (data: unknown) => {
      if (!this.isDestroyed) {
        this.handleBattleEnd(
          data as { winnerId: string; newLevels: { [userId: string]: number } },
        );
      }
    });
  ...
  private handleBattleStart(data: {
    roomId: string;
    ...
    console.log(
      "- Is my turn:",
      this.battleState.currentTurnOwnerId === user?.id,
    );
  ...
  private handleBattleEnd(data: {
    ...
    if (
      ...
      console.warn(
        "Scene inactive or not initialized, skipping battle end handling",
      );
      return;
    }
  ...
  private handleBattleTimeout(data: {
    reason: string;
    readyCount: number;
    expectedCount: number;
    timestamp: string;
    ...
    if (
      ...
      console.warn(
        "Scene inactive or not initialized, skipping battle timeout handling",
      );
      return;
    }
  ...
  private updateTurnIndicator() {
    ...
    if (
      ...
      console.warn(
        "Scene inactive or UI elements not available, skipping turn indicator update",
      );
      return;
    }
