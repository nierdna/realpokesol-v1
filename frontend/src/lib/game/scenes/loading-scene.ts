...
export class LoadingScene extends Phaser.Scene {
  ...
  private createLoadingUI() {
    ...
    // Title
    this.add
      .text(width / 2, height / 2 - 100, "Pokémon Summon Arena", {
        fontSize: "48px",
        color: "#fbbf24",
        fontStyle: "bold",
      })
  ...
  private async connectToServer() {
    try {
      ...
      socketManager.on("connect_error", (error: unknown) =>
        this.onConnectionError(error),
      );
      socketManager.on("disconnect", (reason: unknown) =>
        this.onDisconnect(reason as string),
      );
  ...
  private onConnectionError(error: unknown) {
    ...
    const retryButton = this.add
      .text(width / 2, height / 2 + 150, "Retry Connection", {
        fontSize: "20px",
        color: "#8b5cf6",
